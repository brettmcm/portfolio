...
const image = (item, eager = false) => `<img src="${escape(item.src)}" alt="${escape(item.alt)}"${item.width && item.height ? ` width="${item.width}" height="${item.height}"` : ''} loading="${eager ? 'eager' : 'lazy'}" decoding="async"${eager ? ' fetchpriority="high"' : ''}>`;
const renderers = {
  ...
  media: b => `<div class="case-media case-media--${escape(b.layout || 'single')}"${b.layout === 'cycle' ? ' data-image-cycle data-cycle-interval="1000" aria-label="Category references"' : ''}>${b.images.map((item, index) => `<figure${item.span ? ` style="grid-column:${escape(item.span)}"` : ''}${b.layout === 'cycle' ? ` class="${index === 0 ? 'is-active' : ''}" aria-hidden="${index === 0 ? 'false' : 'true'}"` : ''}>${image(item)}${item.animated ? '<button class="pdf-button motion-toggle" type="button" aria-pressed="false">Pause animation</button>' : ''}${item.caption ? `<figcaption>${escape(item.caption)}</figcaption>` : ''}</figure>`).join('')}</div>`,
  split: b => b.blocks.map(child => renderBlock(child, true)).join('')
};
export function renderBlock(block, nested = false) {
  ...
  return `<${tag}${block.id ? ` id="${escape(block.id)}"` : ''} class="case-block case-block--${escape(block.type)}${nested ? ' case-block--nested' : ''}${block.width ? ` case-block--${escape(block.width)}` : ''}${block.variant ? ` case-block--${escape(block.variant)}` : ''}">${renderers[block.type](block)}</${tag}>`;
}
